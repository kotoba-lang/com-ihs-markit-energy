(ns ihs-markit-energy.main
  "Kotodama WASM entrypoint for the Ihs_markit_energy clean-room actor (L4) — Clojure port.
  ...
  facts (`ihs_markit_energy.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ihs_markit_energy")
(def tier "L4")
...
(def entity-specs
  [{:entity "Commodity" :plural "commodities" :id-prefix "ihsmarki_com"
    :fields [:symbol :name :unit :exchange]         :required [:symbol :name]
    :coerce {}                       :refs {}}
   {:entity "Price"     :plural "prices"     :id-prefix "ihsmarki_pri"
    :fields [:commodityId :value :currency :asOf] :required [:value :currency]
    :coerce {:value :float}          :refs {:commodityId "Commodity"}}
   {:entity "Contract"  :plural "contracts"  :id-prefix "ihsmarki_con"
    :fields [:commodityId :expiry :lastPrice]     :required [:expiry :lastPrice]
    :coerce {:lastPrice :float}      :refs {:commodityId "Commodity"}}
   {:entity "Series"    :plural "serieses"   :id-prefix "ihsmarki_ser"
    :fields [:commodityId :frequency :source]    :required [:frequency :source]
    :coerce {}                       :refs {:commodityId "Commodity"}}
   {:entity "Inventory" :plural "inventories" :id-prefix "ihsmarki_inv"
    :fields [:commodityId :region :quantity :reportedAt] :required [:region :quantity]
    :coerce {:quantity :float}       :refs {:commodityId "Commodity"}}
   {:entity "Forecast"  :plural "forecasts"  :id-prefix "ihsmarki_for"
    :fields [:commodityId :period :value]         :required [:period :value]
    :coerce {:value :float}          :refs {:commodityId "Commodity"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ihs_markit_energy.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ihs_markit_energy-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ihs_markit_energy-compat/ready)
