(ns ihs-markit-energy.main-test
  "Contract + behavioral test for the ihs-markit-energy-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "ihs_markit_energy-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'ihs-markit-energy.main-test)]
